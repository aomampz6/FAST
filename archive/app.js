...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    // Custom brand logos for the ONU brand picker; unknown brands fall back to a generic icon.
    const ONU_BRAND_ICONS = {
        'Huawei': `<div style="color: #E61D2B; display: flex; justify-content: center; align-items: center; width: 100%; height: 100%;">
        ...
        'ZTE': `<div style="color: #0082CC; font-weight: 900; font-size: 18px; font-family: 'Arial Black', sans-serif; letter-spacing: 1px;">ZTE</div>`,
        'Forth': `<div style="display: flex; flex-direction: column; align-items: center; color: #ED1C24;">
        ...
        'Fiberhome': `<div style="display: flex; flex-direction: column; align-items: center; margin-top: 4px;">
    ...
    window.initOnuSetupFlow = () => {
        ...
        brandSelect.innerHTML = brands.map(brand => `
            <button class="option-btn brand-card" onclick="showOnuConfig('${brand.replace(/'/g, '')}')">
                <div class="brand-icon-wrapper">${ONU_BRAND_ICONS[brand] || '<i data-lucide="monitor"></i>'}</div>
                <span>${brand}</span>
            </button>
        `).join('');
        lucide.createIcons();
