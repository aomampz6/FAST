...
/**
 * ONU and ATA device configs share the exact same data shape and workflow —
 * only the `DeviceType` value differs — so this one component drives both
 * admin tabs, locked to whichever device type its route passes in
 * (AdminOnuConfigsTab / AdminAtaConfigsTab). Each tab therefore only ever
 * shows and edits its own device type; there is no cross-device picker here
 * anymore (there used to be one shared tab with an ONU/ATA toggle — see the
 * two thin wrapper files for how the split routes into this component).
 */
export default function AdminDeviceConfigsTab({ deviceType }) {
    const {
        configs,
        loading,
        error,
    ...
    // Brands are scoped to this tab's device type — an ONU brand list should
    // not offer ATA brands and the other way round.
    const brandOptions = useMemo(() => {
        const set = new Set(
            configs.filter((c) => (c.DeviceType || 'ONU') === deviceType && c.Brand).map((c) => c.Brand)
        );
    ...
    const modelOptions = useMemo(() => {
        const set = new Set(
            configs
                .filter((c) => (c.DeviceType || 'ONU') === deviceType && c.Brand === form.Brand && c.Model)
                .map((c) => c.Model)
        );
        return Array.from(set).sort((a, b) => a.localeCompare(b, 'en', { numeric: true }));
    }, [configs, deviceType, form.Brand]);

    // Topics for this tab's device type, in the admin-managed order. The Mode
    // dropdown is driven entirely by this managed list — in its curated order
    // — so it always matches "จัดการหัวข้อการตั้งค่า" exactly. A record whose
    // Mode isn't (or isn't yet) in that list falls back to the free-text box
    // instead of silently mismatching an option (see startEdit below).
    const topicsForDeviceType = useMemo(
        () => topics.filter((t) => (t.DeviceType || 'ONU') === deviceType),
        [topics, deviceType]
    );
    ...
    function startEdit(item) {
        ...
        const managedLabels = topicsForDeviceType.map((t) => t.Label);
        setAddingMode(!!item.Mode && !managedLabels.includes(item.Mode));
    }
    ...
    async function handleDelete(id) {
        if (!window.confirm(`ต้องการลบการตั้งค่า ${deviceType} นี้ใช่หรือไม่?`)) return;
        try {
    ...
    async function handleAddTopic(e) {
        ...
        try {
            await createModeTopic({
                ...
                DeviceType: deviceType,
                Order: topicsForDeviceType.length,
            });
            setNewTopicLabel('');
        } catch (err) {
    ...
    async function moveTopic(topic, direction) {
        const list = topicsForDeviceType;
    ...
    return (
        <div className="admin-section">
            <div className="admin-form">
                <h3>จัดการหัวข้อการตั้งค่า (Mode) — {deviceType}</h3>
                <p className="hint">
                    หัวข้อเหล่านี้เป็นตัวเลือกกลางที่ใช้ร่วมกันทุกยี่ห้อ/รุ่นของ {deviceType} —
                ...
                {topicsLoading && topics.length === 0 ? (
                    ...
                    <ul className="topic-manage-list">
                        {topicsForDeviceType.map((topic, index) => (
                                    ...
                                    <>
                                        <div className="topic-manage-reorder">
                                            ...
                                            <button
                                                type="button"
                                                onClick={() => moveTopic(topic, 1)}
                                                disabled={index === topicsForDeviceType.length - 1}
                                                title="เลื่อนลง"
                                                aria-label="เลื่อนลง"
                                            >
                                                <ArrowDown size={14} />
                        ...
                        {topicsForDeviceType.length === 0 && (
                            <li className="topic-manage-empty">ยังไม่มีหัวข้อสำหรับ {deviceType}</li>
                        )}
                    </ul>
                )}
                ...
                <form className="admin-inline-field" onSubmit={handleAddTopic} style={{ marginTop: 12 }}>
                    <input
                        ...
                        placeholder={`เพิ่มหัวข้อใหม่สำหรับ ${deviceType} เช่น ตั้งค่า Voice / SIP`}
                    />
                    <button type="submit">
            ...
            <form className="admin-form" onSubmit={handleSubmit}>
                <h3>{editingId ? `แก้ไขการตั้งค่า ${deviceType}` : `เพิ่มการตั้งค่า ${deviceType} ใหม่`}</h3>
                        ...
                        {addingBrand || brandOptions.length === 0 ? (
                            <div className="admin-inline-field">
                                <input
                                    ...
                                    placeholder={`ยี่ห้อ ${deviceType} ใหม่`}
                                    required
                    ...
                    <div className="admin-card-header">
                        ...
                        <div>
                            <div className="admin-card-title-row">
                                <h3>รายการตั้งค่า {deviceType} ทั้งหมด</h3>
